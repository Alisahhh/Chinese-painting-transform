...
class CycleGAN:
    ...
    def model(self):
        ...
        # ink_loss
        ink_loss_D = self.discriminator_loss(self.D_Y, self.y_image, fake_y, gan="ink_loss")
        ink_loss_G = self.generator_loss(self.D_Y, self.y_image, gan="ink_loss")
        # identity loss
        ...
        D_Y_loss = self.discriminator_loss(self.D_Y, self.y_image, self.fake_y, gan=cfg.gan) + ink_loss_D
        ...
        tf.summary.scalar('loss/id', id_loss_D)
        tf.summary.scalar('loss/id', id_loss_G)
    ...
    def generator_loss(self, D, fake_y, gan='lsgan'):
        if gan == 'lsgan':
            ...
            loss = 0.05 * tf.reduce_mean(tf.squared_difference(D(fake_y_blur), cfg.REAL_LABEL))
